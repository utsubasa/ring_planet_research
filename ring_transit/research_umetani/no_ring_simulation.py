...
def calc_obs_transit_time(
    ...
    plt.tight_layout()
    os.makedirs(
                f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/calc_obs_transit_time/",
                exist_ok=True,
                )
    plt.savefig(
        f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/calc_obs_transit_time/{TOInumber}.png"
    )
    plt.close()
    ...
    if np.isnan(ts * res.stderr) == False:
        ...
        plt.tight_layout()
        os.makedirs(
                f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/estimate_period/",
                exist_ok=True,
                )
        plt.savefig(
            f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/estimate_period/{TOInumber}.png"
        )
        # plt.show()
        plt.close()
    ...
    if np.all(inverse_mask) == True:
        ...
        if folded_lc == True:
            try:
                ...
                os.makedirs(
                    "/Users/u_tsubasa/work/ring_planet_research/ring_transit/research_umetani/fitting_result/data/no_ring_simulation_TOI495.01/folded_lc/outliers",
                    exist_ok=True,
                )
                outliers.write(
                    f"/Users/u_tsubasa/work/ring_planet_research/ring_transit/research_umetani/fitting_result/data/no_ring_simulation_TOI495.01/folded_lc/outliers/{TOInumber}.csv"
                )
            except ValueError:
                pass
            outliers = []
            ...
            if transit_and_poly_fit == False:
                os.makedirs(
                    f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/each_lc/transit_fit/{TOInumber}",
                    exist_ok=True,
                )
                plt.savefig(
                    f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/each_lc/transit_fit/{TOInumber}/{TOInumber}_{str(i)}.png",
                    header=False,
                    index=False,
                )
                ...
                os.makedirs(
                    f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/each_lc/{savedir}/{TOInumber}",
                    exist_ok=True,
                )
                plt.savefig(
                    f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/each_lc/{savedir}/{TOInumber}/{TOInumber}_{str(i)}.png",
                    header=False,
                    index=False,
                )
                # plt.savefig(f'{homedir}/fitting_result/figure/each_lc/bls/{TOInumber}/{TOInumber}_{str(i)}.png', header=False, index=False)
...
def curve_fitting(each_lc, duration, res=None):
    ...
    result.plot()
    os.makedirs(
        f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/curvefit/{TOInumber}", exist_ok=True
    )
    plt.savefig(
        f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/curvefit/{TOInumber}/{TOInumber}_{str(i)}.png"
    )
...
def curvefit_normalize(each_lc, poly_params, savedir):
    ...
    each_lc.errorbar()
    os.makedirs(
        f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/each_lc/after_curvefit/{TOInumber}",
        exist_ok=True,
    )
    plt.savefig(
        f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/each_lc/after_curvefit/{TOInumber}/{TOInumber}_{str(i)}.png"
    )
    plt.close()
    os.makedirs(
        f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/each_lc/{savedir}/{TOInumber}",
        exist_ok=True,
    )
    each_lc.write(
        f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/each_lc/{savedir}/{TOInumber}/{TOInumber}_{str(i)}.csv"
    )
    return each_lc
...
def folding_lc_from_csv(TOInumber, loaddir, savedir):
    ...
    plt.tight_layout()
    os.makedirs(
        f"/Users/u_tsubasa/Dropbox/ring_planet_research/folded_lc/figure/no_ring_simulation_TOI495.01/{savedir}",
        exist_ok=True,
    )
    plt.savefig(
        f"/Users/u_tsubasa/Dropbox/ring_planet_research/folded_lc/figure/no_ring_simulation_TOI495.01/{savedir}/{TOInumber}.png"
    )
    # plt.show()
    plt.close()
    os.makedirs(
        f"/Users/u_tsubasa/work/ring_planet_research/ring_transit/research_umetani/fitting_result/data/no_ring_simulation_TOI495.01/folded_lc/{savedir}",
        exist_ok=True,
    )
    cleaned_lc.write(
        f"/Users/u_tsubasa/work/ring_planet_research/ring_transit/research_umetani/fitting_result/data/no_ring_simulation_TOI495.01/folded_lc/{savedir}/{TOInumber}.csv"
    )
...
def make_simulation_data():
    ...
    ###リングなしシミュレーション
    p_names = ["t0", "per", "rp", "a", "b", "ecc", "w", "q1", "q2"]
    params = transit_params_setting(pdic_saturnlike['rp_rs'], pdic_saturnlike['porb'])
    params_batman = set_params_batman(params, p_names)
    m = batman.TransitModel(params_batman, t)  # initializes model
    model = m.light_curve(params_batman)  # calculates light curve
    ymodel = model + np.random.randn(len(t))*0.001 + np.sin( (t/0.6 +1.2*np.random.rand()) * np.pi)*0.001
    ...
    os.makedirs(
                f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/before_process/each_lc",
                exist_ok=True,
            )
    plt.savefig(f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/before_process/each_lc/{i}.png")
    #plt.show()
    plt.close()
    os.makedirs(
                f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/before_process/each_lc",
                exist_ok=True,
            )
    each_lc.write(f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/before_process/each_lc/{i}.csv")
...
for TOI in [495.01]:
    ...
    for i in range(54):
        ...
        if data_survival_rate < 0.9:
            if data_survival_rate != 0.0:
                ...
                os.makedirs(
                    f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/error_lc/under_90%_data/calc_t0/{TOInumber}",
                    exist_ok=True,
                )
                plt.savefig(
                    f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/error_lc/under_90%_data/calc_t0/{TOInumber}/{TOInumber}_{str(i)}.png"
                )
                plt.close()
            #t0list.append(mid_transit_time)
            t0errlist.append(np.nan)
        ...
        os.makedirs(
            f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/each_lc/modelresult/1stloop/{TOInumber}",
            exist_ok=True,
        )
        with open(
            f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/each_lc/modelresult/1stloop/{TOInumber}/{TOInumber}_{str(i)}.txt",
            "a",
        ) as f:
    ...
    fold_res = folding_lc_from_csv(
        ...
        loaddir=f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/each_lc/calc_t0",
    ...
    os.makedirs(
        f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/folded_lc/modelresult/1stloop/{TOInumber}",
        exist_ok=True,
    )
    with open(
        f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/folded_lc/modelresult/1stloop/{TOInumber}/{TOInumber}_folded.txt",
        "a",
    ) as f:
    ...
    for i in range(54):
        ...
        """
        each_table = ascii.read(f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/before_process/each_lc/{i}.csv")
        each_lc = lk.LightCurve(data=each_table)
        """解析中断条件を満たさないかチェック"""
        data_survival_rate = calc_data_survival_rate(each_lc, duration)
        if data_survival_rate < 0.9:
            if data_survival_rate != 0.0:
                ...
                ax.set_title(f"{data_survival_rate:4f} useable")
                os.makedirs(
                    f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/error_lc/under_90%_data/obs_t0/{TOInumber}",
                    exist_ok=True,
                )
                plt.savefig(
                    f"{homedir}/fitting_result/figure/no_ring_simulation_TOI495.01/error_lc/under_90%_data/obs_t0/{TOInumber}/{TOInumber}_{str(i)}.png"
                )
                plt.close()
            t0list.append(mid_transit_time)
            t0errlist.append(np.nan)
        ...
        while True:
            ...
            os.makedirs(
                f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/each_lc/modelresult/2ndloop/{TOInumber}",
                exist_ok=True,
            )
            with open(
                f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/each_lc/modelresult/2ndloop/{TOInumber}/{TOInumber}_{str(i)}.txt",
                "a",
            ) as f:
    ...
    fold_res = folding_lc_from_csv(
        TOInumber,
        loaddir=f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/each_lc/obs_t0/",
        savedir="obs_t0",
    )
    # fold_res = folding_lc_from_csv(TOInumber, loaddir=f'{homedir}/fitting_result/data/each_lc/calc_t0', savedir='calc_t0_2ndloop')
    os.makedirs(
        f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/folded_lc/modelresult/2ndloop/{TOInumber}",
        exist_ok=True,
    )
    with open(
        f"{homedir}/fitting_result/data/no_ring_simulation_TOI495.01/folded_lc/modelresult/2ndloop/{TOInumber}/{TOInumber}_folded.txt",
        "a",
    ) as f:
        print(lmfit.fit_report(fold_res), file=f)
