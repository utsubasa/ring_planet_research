...
def make_simulation_data():
    ...
    plt.close()
    os.makedirs(
                f"{homedir}/fitting_result/data/simulation_TOI495.01/before_process/each_lc",
                exist_ok=True,
            )
    each_lc.write(f"{homedir}/fitting_result/data/simulation_TOI495.01/before_process/each_lc/{i}.csv")
...
for TOI in [495.01]:
    ...
    for i in range(54):
        ...
        """
        each_table = ascii.read(f"{homedir}/fitting_result/data/simulation_TOI495.01/before_process/each_lc/{i}.csv")
        each_lc = lk.LightCurve(data=each_table)
        """解析中断条件を満たさないかチェック"""
        data_survival_rate = calc_data_survival_rate(each_lc, duration)
        if data_survival_rate < 0.9:
