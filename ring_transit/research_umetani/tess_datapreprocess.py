...
def preprocess_each_lc(lc, duration, period, transit_time, transit_time_list, TOInumber):
    ...
    for i, mid_transit in enumerate(transit_time_list):
        ...
        out_transit2 = each_lc[(each_lc['time'].value < (transit_time+period*i)-(duration/2)) | (each_lc['time'].value > (transit_time+period*i)+(duration/2))]
        out_transit.scatter(color="blue")
        out_transit2.scatter(color="red")
        plt.show()
        import pdb; pdb.set_trace()
